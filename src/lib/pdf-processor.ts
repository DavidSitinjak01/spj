...
// --- Polyfill browser APIs for pdfjs-dist on Vercel serverless ---
// pdfjs-dist requires DOMMatrix (a browser API) which is not available
// in Node.js serverless environments like Vercel. We polyfill it here
// BEFORE any pdfjs-dist imports happen.
...
export function ensureDOMPolyfills() {
  ...
  if (typeof globalThis.DOMMatrix === 'undefined') {
    try {
      // Try to import the dommatrix polyfill package (CJS)
  ...
  // Polyfill DOMPoint (may also be needed)
  ...
  // Polyfill DOMRect (may also be needed)
